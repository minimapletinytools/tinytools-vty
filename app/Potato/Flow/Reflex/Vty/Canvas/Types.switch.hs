...
data TrackedDrag t a = TrackedDrag {
  ...
  -- | this doesn't actually prevent the drag stream from firing. If you don't want drag events after a cancel, gate promptly with _trackedDrag_dragging
...
captureTrackedDrag' orig@TrackedDrag {..} didCaptureEv = do
  ...
  isCapturingDyn <- foldDynMaybe isCapturingDyn_foldfn False
    $ align (alignEitherAssert "capture drag" (fmap (_drag2_state . snd) _trackedDrag_drag) _trackedDrag_cancel) didCaptureEv
...
switchTrackedDrag dtd = r where
  -- this is the only one that really needs to happen promptly
...
-- | capture a TrackedDrag, returns uncaptured drags
captureTrackedDrag :: forall t m a. (Reflex t, MonadFix m, MonadHold t m)
  => TrackedDrag t a
  -> Event t () -- ^ captures the whole drag if this event fires at the same time as a DragStart
  -> m (TrackedDrag t a) -- ^ uncaptured drags
captureTrackedDrag orig didCaptureEv = do
  tddyn <- captureTrackedDrag' orig didCaptureEv
  return $ switchTrackedDrag (fmap fst tddyn)

-- | use this helper method to create dynamics that follow drags until it ends or is cancelled
...
-- potato filter for tracked drag events
